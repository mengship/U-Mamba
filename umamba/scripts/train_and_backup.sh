...
RESULTS_DIR="/hy-tmp/U-Mamba/results"
...
# TRAIN_CMD='nnUNetv2_train 705 3d_fullres 0 -tr nnUNetTrainerUMambaBot_50epochs' bash /root/train_and_backup.sh
TRAIN_CMD="nnUNetv2_train 705 3d_fullres 0 -tr nnUNetTrainerUMambaBot_50epochs"
LOGFILE="$RESULTS_DIR/train_and_backup_$(date '+%Y%m%d-%H%M%S').log"

echo "[train_and_backup] Log: $LOGFILE"

# Run training
echo "[train_and_backup] Starting training at $(date)" | tee -a "$LOGFILE"
set -o pipefail
eval "$TRAIN_CMD" 2>&1 | tee -a "$LOGFILE"
TRAIN_EXIT=${PIPESTATUS[0]:-$?}
if [ "$TRAIN_EXIT" -ne 0 ]; then
  echo "[train_and_backup] Training exited with code $TRAIN_EXIT — aborting backup." | tee -a "$LOGFILE"
  exit $TRAIN_EXIT
fi
