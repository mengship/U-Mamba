...
class nnUNetTrainerUMambaEncRTHD_NoCrossViewGate_Clean(nnUNetTrainer):
    """
    Clean NoCrossView Ablation: 基于StageAwareDecoder_SkipCalibration，仅关闭cross_view_interaction

    配置变化（相对于StageAwareDecoder_SkipCalibration）：
    - Encoder: cross_view_interaction = False （唯一变化）
    - Decoder: cross_view_interaction = False （唯一变化）

    其他配置完全相同：
    - Encoder use_local_window=True
    - Decoder use_local_window=False
    ...
    实验目的：
    纯净的跨视图交互消融，验证cross_view_interaction的独立贡献

    使用方法:
        nnUNetv2_train DATASET_ID CONFIG FOLD -tr nnUNetTrainerUMambaEncRTHD_NoCrossViewGate_Clean
    """
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        if len(configuration_manager.patch_size) == 3:
            # 完全复制StageAwareDecoder_SkipCalibration，仅修改cross_view_interaction
            model = get_umamba_enc_rthd_3d_from_plans(
                ...
                rthd_config_encoder={
                    ...
                    "cross_view_interaction": False,  # 修改点：关闭跨视图交互
        ...
        print("=" * 80)
        print("UMambaEnc_RTHD: NoCrossViewGate Clean Ablation")
        print("=" * 80)
        print("配置差异（相对于StageAwareDecoder_SkipCalibration）:")
        print("  - Encoder cross_view_interaction: True → False")
        print("  - Decoder cross_view_interaction: True → False")
        print()
        print("编码器配置:")
        print(f"  - use_local_window: True")
        print(f"  - cross_view_interaction: False ← 消融点")
        print("  - RTHD stages: [0, 1, 2, 3, 4]")
        print()
        print("解码器配置:")
        print(f"  - use_local_window: False")
        print(f"  - cross_view_interaction: False ← 消融点")
