...
class nnUNetTrainerUMambaEncRTHD_PartialDecoderETSM(nnUNetTrainer):
    """
    CONSERVATIVE VERSION: encoder ETSM + partial decoder ETSM (only low-res stages).

    Only applies ETSM to decoder stages 0, 1, 2 (low resolution: 8³, 16³, 32³).
    High-resolution stages 3, 4 (64³, 128³) use standard convolutions to avoid OOM.

    This is a safer alternative when full decoder ETSM causes OOM on 24GB GPUs.
    """
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        return get_umamba_enc_rthd_3d_from_plans(
            ...
            decoder_rthd_mode="partial",  # 只在低分辨率 stage 使用 RTHD
